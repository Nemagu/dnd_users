...
type emailUserIDToken struct {
	EncryptedEmail  string    `json:"e"`
	EncryptedUserID string    `json:"u"`
	CreatedAt       time.Time `json:"c"`
	Salt            string    `json:"s"`
	Nonce           string    `json:"n"`
}
...
func (s *BcryptEmailCrypter) EncryptEmail(email string) (string, error) {
	salt, err := makeSalt()
	if err != nil {
		return "", err
	}
...
func (s *BcryptEmailCrypter) EncryptEmailUserID(email string, userID uuid.UUID) (string, error) {
	salt, err := makeSalt()
	if err != nil {
		return "", err
	}
	...
	encryptedEmail := crypter.Seal(nil, nonce, []byte(email), nil)
	encryptedUserID := crypter.Seal(nil, nonce, userID[:], nil)

	token := emailUserIDToken{
		EncryptedEmail:  base64.StdEncoding.EncodeToString(encryptedEmail),
		EncryptedUserID: base64.StdEncoding.EncodeToString(encryptedUserID),
		CreatedAt:       time.Now(),
		Salt:            base64.StdEncoding.EncodeToString(salt),
		Nonce:           base64.StdEncoding.EncodeToString(nonce),
	}
...
func (s *BcryptEmailCrypter) DecryptEmail(token string) (string, error) {
...
func (s *BcryptEmailCrypter) DecryptEmailUserID(token string) (string, uuid.UUID, error) {
	marshaledToken, err := base64.URLEncoding.DecodeString(token)
	if err != nil {
		return "", uuid.Nil, fmt.Errorf("%w: %s", application.ErrInternal, err)
	}

	var t emailUserIDToken
	if err := json.Unmarshal(marshaledToken, &t); err != nil {
		return "", uuid.Nil, fmt.Errorf("%w: %s", application.ErrInternal, err)
	}
	...
	encryptedUserID, err := base64.StdEncoding.DecodeString(t.EncryptedEmail)
	if err != nil {
		return "", uuid.Nil, fmt.Errorf("%w: %s", application.ErrInternal, err)
	}
	...
	decryptedUserID, err := crypter.Open(nil, nonce, encryptedUserID, nil)
	if err != nil {
		return "", uuid.Nil, fmt.Errorf("%w: %s", application.ErrInternal, err)
	}

	userID, err := uuid.FromBytes(decryptedUserID)
	if err != nil {
		return "", uuid.Nil, fmt.Errorf("%w: %s", application.ErrInternal, err)
	}

	return string(decryptedEmail), userID, nil
}
